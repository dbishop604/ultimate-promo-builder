...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!result.valid) {
      const gumroadMessage = String(result.data?.message || result.data?.error || '').slice(0, 240);
      console.error('Gumroad license rejected:', JSON.stringify({
      ...
      const detail = gumroadMessage ? ` Gumroad says: ${gumroadMessage}` : ` Gumroad returned HTTP ${result.status}.`;
      return res.status(401).json({ ok: false, message: `That license key could not be verified.${detail}` });
    }
